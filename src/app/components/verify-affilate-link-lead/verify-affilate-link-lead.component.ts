...
@Component({
  selector: 'app-verify-affilate-link-lead',
  templateUrl: './verify-affilate-link-lead.component.html',
  styleUrls: ['./verify-affilate-link-lead.component.scss']
})
export class VerifyAffilateLinkLeadComponent implements OnInit {
  token: string = '';
  color: string = '#fa5440';
  exactColor: string = '#fa5440';
    ...
    async verifyToken() {
        try {
            const res$ = this.apiService.postReq(API_PATH.VERIFY_AFFILATE_LINK, { token: this.token, color: this.exactColor }, '', '');
    ...
    saveUserDetails(response: any): void {
        ...
        this.router.navigate([`/create-lead-affilate-link`], { queryParams:{token: response.data.token }})
    }
    // loader color
    ngDoCheck(): void {
