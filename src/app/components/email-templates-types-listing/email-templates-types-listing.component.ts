...
@Component({
    selector: 'app-email-templates-types-listing',
    templateUrl: './email-templates-types-listing.component.html',
    styleUrls: ['./email-templates-types-listing.component.scss', '../../styles/dashboard.scss', '../../styles/predictive-search.scss']
})
export class EmailTemplatesTypesListingComponent implements OnInit {
    ...
    canAddEmailTemplateType: boolean = false;
    canUpdateEmailTemplateType: boolean = false;
    canDeleteEmailTemplateType: boolean = false;
    canViewEmailTemplateType: boolean = false;
    ...
    ngOnInit(): void {
        ...
        this.canAddEmailTemplateType = this.authService.hasPermission('document-type-create');
        this.canUpdateEmailTemplateType = this.authService.hasPermission('document-type-update');
        this.canDeleteEmailTemplateType = this.authService.hasPermission('document-type-delete');
        this.canViewEmailTemplateType = this.authService.hasPermission('document-type-view');
    ...
    async createLeadStatusSubmit(): Promise<any> {
        ...
        if (this.leadStatusForm.valid) {
            ...
            try {
                ...
                const res$ = this.apiService.postReq(API_PATH.EMAIL_TEMPLATE_TYPE_CREATE, data,  'email', 'template-type-create');
    ...
    */
    async getDocumentTypesList(): Promise<any> {
        try {
            ...
            this.commonService.showSpinner();
            const res$ = this.apiService.getReq(API_PATH.EMAIL_TEMPLATE_TYPE_LIST + url, 'email', 'template-type-list');
            let response = await lastValueFrom(res$);
  ...
  */
    ...
    async deleteLeadStatus(user: any) {
        ...
        try {
            let url = `?id=${user.id}&name=${user.name}&is_deleted=${this.deleted}`;
            this.commonService.showSpinner();
            const res$ = this.apiService.getReq(API_PATH.EMAIL_TEMPLATE_TYPE_DELETE + url, 'email', 'template-type-delete');
            let response = await lastValueFrom(res$);
    ...
    async updateStatus(status: string, user: any, input: any, i: number) {
        try {
            const res$ = this.apiService.postReq(API_PATH.EMAIL_TEMPLATE_TYPE_UPDATE_STATUS, { name: user.name, id: user.id, status: status , is_deleted: this.updatestatus}, 'email', 'template-type-update');
    ...
    async updateLeadStatus(): Promise<any> {
        ...
        if (this.EditleadSouceForm.valid) {
            ...
            try {
                ...
                const res$ = this.apiService.postReq(API_PATH.EMAIL_TEMPLATE_TYPE_UPDATE, data, 'email', 'template-type-update');
                let response = await lastValueFrom(res$);
                if (response) {
                    this.commonService.showSuccess(response.message);
                    this.modal.close
    ...
    patchValue(user: any) {
        this.EditleadSouceForm.get('name')?.patchValue(user.name);
