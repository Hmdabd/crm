...
export class EmailTemplateListComponent implements OnInit {
    ...
    tabView: string = 'System';
    emailtemplatesType: string = 'System';
...
for (let index = 0; index < tds.length; index++) {
 tds[index].style.setProperty('--custom',`${this.color}`);
    ...
    getTemplateList() {
        this.appliedFilter = JSON.parse(JSON.stringify(this.tempFilter));
        this.tabView = this.appliedFilter['Template Type']?.value?.name;
        this.emailtemplatesType = this.appliedFilter['Template Type']?.value?.name;
        this.onSearch();
    ...
    async getEmailTemplateList(): Promise<any> {
        try {
            let url = `?sort_by=${this.search.sortby}&dir=${this.search.order}&page_limit=${this.usersListLimit}&page=${this.userListPage}&type=${this.emailtemplatesType}`;
            ...
            if ((this.appliedFilter['Template Type']?.value?.name)) {
                url = `${url}&template_type=${this.appliedFilter['Template Type']?.value?.name}`;
            }
    ...
    // customTemplates() {
    //     this.tabView = 'Custom';
    //     this.emailtemplatesType = 'Custom';
    //     this.getEmailTemplateList()

    // }
    systemTemplates(templatename: any) {
        this.tabView = templatename;
        this.emailtemplatesType = templatename;
        this.getEmailTemplateList()
    }
    // leadStatusTemplates(){
    //     this.tabView  = 'Lead Status based';
    //     this.emailtemplatesType = 'Lead Status based';
    //     this.getEmailTemplateList()

    // }
    gettemplateName(templateName: any){
        if(this.emailtemplatesType == templateName){
            return false
        }else{
            return true
        }
