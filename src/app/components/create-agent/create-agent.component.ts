...
export class CreateAgentComponent implements OnInit {
    ...
    initAddAgentForm(): void {
        this.addAgentForm = this.fb.group({
            ...
            last_name: ['', [Validators.required, Validators.pattern(Custom_Regex.spaces), Validators.pattern(Custom_Regex.lettersOnly),
    ...
    openrestoreUserpopup(message: any) {
        Swal.fire({
            title: message,
        ...
        if (this.addAgentForm.valid) {
            try {
                ...
                let data = {
                    ...
                    already_exist: this.userExistvalue,
   ...
   async getEmailData(): Promise<void>{
    try{
        this.commonService.showSpinner();
        ...
        const res$ = this.apiService.postReq(API_PATH.AGENT_DETAIL, data, 'user', 'list');
        let response = await lastValueFrom(res$);

        if(response && response.data){
            let responseData = response.data
            let firstName = responseData?.first_name;
            let lasttName = responseData?.last_name;
            let phoneNumber = responseData?.phone_number;
            let status = responseData?.status;
           if(firstName != ''){
          this.addAgentForm.patchValue({ first_name:firstName})
           }
           if(lasttName != ''){
            this.addAgentForm.patchValue({last_name:lasttName})
           }
           if(phoneNumber != ''){
            this.addAgentForm.patchValue({ phone_number:phoneNumber})

           }
           if(status != ''){
            this.addAgentForm.patchValue({ status:status})
