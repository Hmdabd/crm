...
export class LeadActivitiesComponent implements OnInit {
    ...
    canViewActivity: boolean = false;
    canDeleteActivity: boolean = false;
    constructor(
    ...
    ngOnInit(): void {
        ...
        this.getUserDetails();
        this.canViewActivity = this.authService.hasPermission('lead-activity-view');
        this.canDeleteActivity = this.authService.hasPermission('lead-activity-delete');
    }
    ngDoCheck():void {
...
for (let index = 0; index < tds.length; index++) {
 tds[index].style.setProperty('--custom',`${this.color}`);
    ...
    async deleteActivity(id: any): Promise<void> {
        try {
            this.commonService.showSpinner();
            const res$ = this.apiService.postReq(API_PATH.DELETE_ACTIVITY, { id: id },  'lead', 'activity-delete');
            let response = await lastValueFrom(res$);
            if (response && response.data) {
                this.commonService.showSuccess(response.message);
                this.leadActivityList = this.leadActivityList.filter(el => !id.includes(el.id));
            }
            this.commonService.hideSpinner();
        } catch (error: any) {
            this.commonService.hideSpinner();
