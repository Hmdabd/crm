import { Component, ViewChild, OnInit, OnDestroy, TemplateRef, ChangeDetectorRef } from '@angular/core';
   ...
   */
    initEmailTemplateForm() {
        this.emailTemplateForm = this.fb.group({
            ...
            // to:['', [Validators.required,Validators.pattern(Custom_Regex.email)]],
            // , this.ValidateEmails, Validators.pattern(Custom_Regex.spaces)
            ...
            send_on: [''],
            merchant_document_status:[false],
    ...
    async emailTemplateFormSubmit(): Promise<void> {
        if (this.editableEmail) {
            this.emailTemplateForm.get('to')?.setValidators([Validators.required, Validators.pattern(Custom_Regex.email)]);
            this.emailTemplateForm.get('to')?.updateValueAndValidity();
        } else {
            this.emailTemplateForm.get('email')?.setValidators([Validators.required]);
            this.emailTemplateForm.get('email')?.updateValueAndValidity();
        }
        ...
        if (this.emailTemplateForm.valid && this.formFileArray.valid) {
            ...
            try {
                ...
                formData.append('email', this.editableEmail ? this.emailTemplateForm.get('to')?.value : this.emailTemplateForm.get('email')?.value),
                ...
                // formData.append('schedule', this.emailTemplateForm.get('schedule')?.value);
                let merchant_docs_value:any = this.emailTemplateForm.value.merchant_document_status?1:0;
                formData.append('merchant_document_status',merchant_docs_value);
    ...
    async getEmailTemplateOptions() {
        try {
            let url = '?sort_by=template_name&dir=ASC';
            ...
            if (response && response.data) {
                ...
                //25th oct 2023 // show hot app out default selected
                let getHotAppOutTemplateId = this.emailTemplatesList.filter((e)=>{if(e.is_selected == 1){
                    return e.id
                    
                }});
                setTimeout(() => {
                    if(getHotAppOutTemplateId)
                    this.emailTemplateForm.patchValue({
                        template_name:getHotAppOutTemplateId[0]?.id
                      }) 
                      if(getHotAppOutTemplateId[0]?.editable){
                        this.emailTemplateForm.patchValue({
                            to: this.lead?.email
                        })
    ...
    async getEmailTemplateData(template: any) {
        if (template) {
            try {
                ...
                let temp = this.emailTemplatesList.find((e) => e.id === this.emailTemplateForm.value.template_name);
                if (temp) {
                    if (temp.editable == 1) {
                        this.emailTemplateForm.patchValue({
                            to: this.lead?.email
 ...
 */
    ...
    getLeadBasicDetails(leadData: any) {
        ...
        this.emailTemplateForm.patchValue({merchant_document_status:leadData?.merchant_document_status})
