...
@Component({
  selector: 'app-lead-submission-updates',
  templateUrl: './lead-submission-updates.component.html',
  styleUrls: ['./lead-submission-updates.component.scss']
})
export class LeadSubmissionUpdatesComponent implements OnInit {
  leadID: string = '';
  updatesList: Array<any> = [];
  search: string = '';
  submissionupdatepage: number = 1;
  limit: number = 10;
  total: number = 0;
  selectedDate: string = '';
  ...
  getPaginationList() {
        // change index 0 -> 1
      let data = document.getElementsByClassName('ngx-pagination')[2]?.getElementsByTagName('li');
   ...
   */
  async getLeadSubmissionUpdates() {
      try {
          this.commonService.showSpinner();
          let res$ = this.apiService.postReq(API_PATH.LEAD_UPDATES, { lead_id: this.leadID, search: this.search, page: this.submissionupdatepage, dateRange: this.selectedDate , type: 'submission_updates'}, 'lead', 'list');
