...
export class LeadEditComponent implements OnInit, AfterViewInit {
    ...
    incorperationstatesList:Array<any>= []
    ...
    ngOnInit(): void {
        ...
        // make first last name conditionaly required :-
        // if(this.userDetails?.company_type == 'funded'){
        //     this.leadDetailsForm.get('first_name')?.setValidators([
        //        Validators.required,
        ...
        //    ]);
        //    this.leadDetailsForm.get('first_name')?.updateValueAndValidity();
        //    this.leadDetailsForm.get('last_name')?.setValidators([
        //        Validators.required,
        //        Validators.pattern(Custom_Regex.spaces),
        //        Validators.pattern(Custom_Regex.lettersWithSpecialChar),
        //        Validators.maxLength(100),
        //        // Validators.minLength(3)
        //    ]);
        //    this.leadDetailsForm.get('last_name')?.updateValueAndValidity();
        //    }

        // Owner 1 first name mandatory for broker case
        //     if(this.userDetails?.company_type == 'broker'){
        //          this.officerInfoForm.get('officer_first_name')?.setValidators([
        //         Validators.required
        //     ]);
        //     this.officerInfoForm.get('officer_first_name')?.updateValueAndValidity();
        //     this.officerInfoForm.get('officer_last_name')?.setValidators([
        //         Validators.required
        //     ]);
        //     this.officerInfoForm.get('officer_last_name')?.updateValueAndValidity();
    ...
    patchBasicDetails() {
        ...
        this.leadDetailsForm.patchValue({
            ...
            advance_id: this.lead?.advance_id,
            ...
            lead_incorporation_state:this.lead?.lead_incorporation_state
        });
    ...
    patchOfficerInfo() {
        this.officerInfoForm.patchValue({
            ...
            officer_dl: this.leadOfficer?.officer_dl,
            officer_job_title:this.leadOfficer?.officer_job_title
        })
    ...
    patchPartnerInfo() {
        this.partnerInfoForm.patchValue({
            ...
            partner_dl: this.leadPartner?.partner_dl,
            partner_job_title: this.leadPartner?.partner_job_title
        })
    }
    ...
    initLeadDetailsForm() {
        this.leadDetailsForm = this.fb.group({
            // Validators.required,
            // [ Validators.pattern(Custom_Regex.spaces), Validators.pattern(Custom_Regex.lettersWithSpecialChar), Validators.maxLength(100)]
            first_name: [''],
            last_name: [''],
            // Validators.minLength(3)
            company_name: ['', [Validators.required, Validators.pattern(Custom_Regex.spaces), Validators.pattern(Custom_Regex.name), Validators.pattern(Custom_Regex.username), Validators.maxLength(100)]],
            ...
            lead_product_sold: ['', [
                // Validators.minLength(3), 
                Validators.pattern(Custom_Regex.spaces), Validators.pattern(Custom_Regex.address), Validators.pattern(Custom_Regex.address2), Validators.maxLength(50)]],
            // lead_use_of_proceed: ['', [
            //     // Validators.minLength(3), 
            //     Validators.pattern(Custom_Regex.spaces), Validators.pattern(Custom_Regex.address), Validators.pattern(Custom_Regex.address2), Validators.maxLength(100)]],
            use_of_funds: ['', [
                    // Validators.minLength(3), 
                    Validators.pattern(Custom_Regex.spaces), Validators.pattern(Custom_Regex.address), Validators.pattern(Custom_Regex.address2), Validators.maxLength(100)]],
            ...
            advance_id: ['', [Validators.pattern(Custom_Regex.spaces)]],
    ...
    initOfficerInfoForm() {
        this.officerInfoForm = this.fb.group({
            officer_first_name: ['', [Validators.required,Validators.pattern(Custom_Regex.spaces), Validators.pattern(Custom_Regex.spaceBetweenWord),
                //  Validators.minLength(3),
                  Validators.maxLength(100)]],
            officer_last_name: ['', [Validators.required,Validators.pattern(Custom_Regex.spaces), Validators.pattern(Custom_Regex.spaceBetweenWord), 
                // Validators.minLength(3),
                 Validators.maxLength(100)]],
            officer_email: ['', [Validators.pattern(Custom_Regex.email),]],
            officer_dob: [null],
            officer_dl:['',[Validators.pattern(/^[a-zA-Z0-9-_]+$/)]],
            officer_job_title:[''],
    ...
    initPartnerInfoForm() {
        this.partnerInfoForm = this.fb.group({
            partner_first_name: ['', [
                // Validators.minLength(3), 
                Validators.pattern(Custom_Regex.spaces), Validators.pattern(Custom_Regex.spaceBetweenWord), Validators.maxLength(100)]],
            partner_last_name: ['', [
                // Validators.minLength(3),
                 Validators.pattern(Custom_Regex.spaces), Validators.pattern(Custom_Regex.spaceBetweenWord), Validators.maxLength(100)]],
            partner_email: ['', [Validators.pattern(Custom_Regex.spaces), Validators.pattern(Custom_Regex.email),]],
            partner_dob: [null],
            partner_dl:['',[Validators.pattern(/^[a-zA-Z0-9]+$/)]],
            partner_job_title:[''],
            partner_address: ['', [
    ...
    */
     ...
     * @author Shine Dezign Infonet Pvt. Ltd.
     */
     async getIncorperationStates() {
        try {
            let usId = 'Pdi0dAr1Y9Y1lBjLZjO5rw=='
            this.commonService.showSpinner();
            const res$ = this.apiService.postReq(API_PATH.STATES, { country_id: usId}, '', '');
            let response = await lastValueFrom(res$);
            if (response && response.data) {
                this.incorperationstatesList = response.data;
                // this.leadDetailsForm.patchValue({ lead_state: "" });
                // if (forall) {
                //     this.offStatesList = response.data;
                //     this.partnerStatesList = response.data;
