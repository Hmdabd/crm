...
export class ProposeSubmissionComponent implements OnInit {
    ...
    //         const res$ = this.apiService.postReq(API_PATH.LENDERS_LIST, { lead_id: this.leadID,propose:1 }, 'propose', 'submission');
    //         let response = await lastValueFrom(res$);
    //         if (response && response.status_code == "200") {
    //             this.submittedlendersList = response.data.lenders;
     ...
     */
        ...
        async getlendersList() {
            try {
                ...
                let response = await lastValueFrom(res$);
                if (response && response.status_code == "200") {
                    this.lendersList = response.data.lender;
                   this.proposeSubForm.patchValue({custom_note:response.data.note}) 
                } else {
                    this.commonService.showError(response.message);
                }
                this.commonService.hideSpinner();
    ...
    async onSubmit() {
        try {
            ...
            if(this.proposeSubForm.valid) {
                const res$ = this.apiService.postReq(API_PATH.PROPOSE_SUBMISSION,{ lead_id: this.leadID, note: this.proposeSubForm.value.custom_note, lender_id: this.proposeSubForm.value.lenders , state_id: this.proposeSubForm.value.state_id }, 'propose', 'submission');
                let response = await lastValueFrom(res$);
                if(response && response.status_code == "200") {
                    this.commonService.showSuccess(response.message);
                    this.router.navigate([`${this.userBaseRoute}/lead-detail/${this.leadID}`]);
    ...
    async getStates(country_id: string) {
        try {
            ...
            if (response && response.data) {
                ...
                setTimeout(() => {
                    this.proposeSubForm.patchValue({state_id: this.lead.encrypted_state_id})
                })
