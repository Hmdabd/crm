...
export class CustomerLeadUpdateComponent implements OnInit {
    ...
    owner1Date: string = '';
    owner2Date: string = '';
    packageOpenedDateSign1: string = '';
    signatureAdoptedDateSign1: string = '';
    packageOpenedDateSign2: string = '';
    signatureAdoptedDateSign2: string = '';
    constructor(
    ...
    async getLeadDetails() {
        try {
            let url = `?sendApp=${'true'}`;
            this.commonService.showSpinner();
            const res$ = this.apiService.getReq(API_PATH.LEAD_EDIT_DETAIL + this.leadID + url, 'lead', 'view');
            let response = await lastValueFrom(res$);
            if (response && response.data) {
                ...
                //         this.owner1Date = moment(date).tz(this.timeZone).format(`${this.dateFormat} hh:mm:ss A`);
                //     } else {
                //         this.owner1Date = moment(this.lead.sign_date).tz(this.timeZone).format(`${this.dateFormat} hh:mm:ss A`);
                //     }
                // }
                // if (this.lead.other_customer_sign_date) {
                //     if (this.userDetails.company_type == 'broker') {
                //         this.owner2Date = moment(date).tz(this.timeZone).format(`${this.dateFormat} hh:mm:ss A`);
                //     } else {
                //         this.owner2Date = moment(this.lead.other_customer_sign_date).tz(this.timeZone).format(`${this.dateFormat} hh:mm:ss A`);
                //     }
                // }
                if (this.lead.sign_date) {
                    this.owner1Date = moment(this.lead.sign_date).tz(this.timeZone).format(`${this.dateFormat} hh:mm:ss A`);
    ...
    patchBasicDetails() {
        ...
        this.leadDetailsForm.patchValue({
            ...
            total_outstanding_balance: this.lead.total_outstanding_balance,
    ...
    initLeadDetailsForm() {
        this.leadDetailsForm = this.fb.group({
            ...
            broker_email: ['', [Validators.pattern(Custom_Regex.email)]],
        })
    }
    ...
    async onLeadFormSubmit() {
        this.leadDetailsForm.markAllAsTouched();
        //   console.log("bhgv", this.leadDetailsForm)
        if (this.leadDetailsForm.value.open_advances == 1) {
        ...
        if (this.leadDetailsForm.valid) {
            try {
                ...
                let data = {
                    ...
                    user_type: 'customer',
                    lead_iso: this.lead.lead_iso,
                    lead_closer: this.lead.lead_closer,
                    updates:this.userDetails.company_type == 'broker'?'broker':'funded'
                }
    ...
    async onOfficerInfoSubmit() {
        ...
        this.commonService.showSpinner();
        this.getDk = this.officerInfoForm.valid
            ...
            try {
                ...
                if (this.userDetails.role === Roles.CUSTOMER) {
                    data.sign = this.customerSign,
                        data.package_opened_at = this.getDate(this.packageOpenedDateSign1),
                        data.signature_adopted_at = this.getDate(this.signatureAdoptedDateSign1),
                        data.package_signed_at = this.getDate(registerDate)
                }
                // this.commonService.showSpinner();
    ...
    initPartnerInfoForm() {
        this.partnerInfoForm = this.fb.group({
            partner_first_name: ['', [
                ...
                // Validators.minLength(3),
                Validators.maxLength(100)]],
            partner_last_name: ['', [Validators.pattern(Custom_Regex.spaceBetweenWord),
            //  Validators.minLength(3),
            Validators.maxLength(100)]],
            ...
            partner_other_address: ['', [Validators.pattern(Custom_Regex.address),
            //  Validators.minLength(3),
    ...
    async signedApplication(): Promise<void> {
        this.getsignedLeadDetails();
    }

    async signed2Application(): Promise<void> {
        try {
            let url = `?sign_date=${this.owner1Date}&other_customer_sign_date=${this.owner2Date}`;
            // let url = `?uploaded_by=${'System'}`;
            this.commonService.showSpinner();
            const res$ = this.apiService.getReq(API_PATH.EXPORT_PDF + `${this.leadID}` + `/${'System'}` + url, 'lead', 'view');
    ...
    async onPartnerInfoSubmit(value: any) {
        if (this.lead?.merchant_document_status && this.userDetails.company_type == 'broker' && value == true) {
            let uploadocarray = []
            for (let index = 0; index < this.uploadedDocsList.length; index++) {

                uploadocarray.push(this.uploadedDocsList[index].actual_type);

            }
            if (this.uploadedDocsList.length && this.formFileArray.length) {
                for (let i = 0; i < this.formFileArray.length; i++) {
                    for (let index = 0; index < this.uploadedDocsList.length; index++) {
                        // if((this.uploadedDocsList[index].actual_type != '3month_bank_statement' && 
                        if ((this.uploadedDocsList[index].actual_type != '3month_bank_statement' && this.filesForm.get('files')['controls'][i].controls.doc_type.value == '3month_bank_statement')) {
            ...
            else if (this.uploadedDocsList.length) {
                // let uploadocarray = []  
                // for (let index = 0; index < this.uploadedDocsList.length; index++) {
                //     console.log("ajsbfj", this.uploadedDocsList[index].actual_type != '3month_bank_statement');
                //     uploadocarray.push( this.uploadedDocsList[index].actual_type);

                //     // if(this.uploadedDocsList[index].actual_type != '3month_bank_statement'){
                //     //     console.log("actual_type",this.uploadedDocsList[index].actual_type != '3month_bank_statement');
                //     //     this.commonService.showError('Please Upload 3 month bank statement')
                //     //            return
                //     // }
                ...
                if (!uploadocarray.includes('3month_bank_statement')) {
                    this.commonService.showError('Please Upload 3 month bank statement')
                    return
                }
        ...
        this.partnerInfoForm.markAllAsTouched();
        if (this.partnerInfoForm.valid && this.formFileArray.valid) {
            try {
                ...
                let registerDate = new Date().toJSON("yyyy/MM/dd HH:mm");
                if (this.userDetails.role === Roles.CUSTOMER && value == false) {
                    // data.sign = this.customerSign,
                    data.sign2 = this.customer2Sign,
                        data.package_opened_at = this.getDate(this.packageOpenedDateSign2),
                        data.signature_adopted_at = this.getDate(this.signatureAdoptedDateSign2),
                        data.package_signed_at = this.getDate(registerDate)
                }
                // this.commonService.showSpinner();
                ...
                if (response && response.data) {
                    this.commonService.showSuccess(response.message);
                    if (this.customer2SignCloseModal == false && value == false) {
                        this.tabView = false;
                    } else {
    ...
    */
    ...
    initBussinessInfoForm() {
        this.bussinessInfoForm = this.fb.group({
            business_first_name: ['', [Validators.pattern(Custom_Regex.spaceBetweenWord), Validators.maxLength(100)]],
            business_last_name: ['', [Validators.pattern(Custom_Regex.spaceBetweenWord), Validators.maxLength(100)]],
    ...
    */
    ...
    async getAssignedOptions() {
        try {
            // this.commonService.showSpinner();
            let url = `&sort_by=${this.search.sortby}&dir=${this.search.order}&assigned_to=assignedTo`;
            const res$ = this.apiService.getReq(API_PATH.LIST_FOR_USERS + `?role=${Roles.UNDERWRITER}` + url, 'user', 'list');
            let response = await lastValueFrom(res$);
            if (response && response.data) {
                this.assineesList = response.data;
            }
    ...
    // getRequiredDocs(){
    //     let document_type = '3month_bank_statement'
    //     if(this.lead?.merchant_document_status){
    //         console.log('entered');


    //         // console.log('gdsh',  this.uploadedDocsList)
    //         // this.uploadedDocsList.map((m:any)=>{
    //         //     console.log(m);
    //         //     // 'Bank Statement'(document_type)
    //         //     if(m?.document_type != 'Bank Statement'){
    //         //         this.commonService.showError('Please Upload 3 month bank statement');
    //         //         return;
    //         //     }
    //         // })
    ...
    //         for (let i = 0; i < this.formFileArray.length; i++) {
    //             this.uploadedDocsList.map((m:any)=>{console.log(m);
    //                 // 'Bank Statement'(document_type)
    //             })
    //              if (this.filesForm.get('files')['controls'][i].controls.doc_type.value != document_type ){
    //             //    this.filesForm.get('files')['controls'][i].controls.doc_type.value;
    //                this.commonService.showError('Please Upload 3 month bank statement');
    //                return;
    ...
    //         }
    //     }
    //     if(this.uploadedDocsList.length){
    //         console.log('test2');

    //         this.uploadedDocsList.map((m:any)=>{
    //             if(m?.document_type != '3month_bank_statement' && m?.document_type != 'Bank Statement'){
...
requiredFieldForBorker(){
  if(this.userDetails.company_type == 'broker'){
    this.leadDetailsForm.get('lead_business_started')?.setValidators([Validators.required]);
    this.leadDetailsForm.get('lead_business_started')?.updateValueAndValidity();

    this.leadDetailsForm.get('lead_federal_tax_id')?.setValidators([Validators.required]);
    this.leadDetailsForm.get('lead_federal_tax_id')?.updateValueAndValidity();

    this.leadDetailsForm.get('lead_business_type')?.setValidators([Validators.required]);
    this.leadDetailsForm.get('lead_business_type')?.updateValueAndValidity();

    this.leadDetailsForm.get('lead_entity_type')?.setValidators([Validators.required]);
    this.leadDetailsForm.get('lead_entity_type')?.updateValueAndValidity();

    this.officerInfoForm.get('officer_ownership')?.setValidators([Validators.required]);
    this.officerInfoForm.get('officer_ownership')?.updateValueAndValidity();

    this.officerInfoForm.get('officer_ssn')?.setValidators([Validators.required]);
    this.officerInfoForm.get('officer_ssn')?.updateValueAndValidity();

    this.officerInfoForm.get('officer_ownership')?.setValidators([Validators.required]);
    this.officerInfoForm.get('officer_ownership')?.updateValueAndValidity();

    this.officerInfoForm.get('officer_dob')?.setValidators([Validators.required]);
    this.officerInfoForm.get('officer_dob')?.updateValueAndValidity();

}
else{
    this.leadDetailsForm.get('lead_federal_tax_id')?.clearValidators();
    this.leadDetailsForm.get('lead_federal_tax_id')?.updateValueAndValidity();

    this.leadDetailsForm.get('lead_entity_type')?.clearValidators
    this.leadDetailsForm.get('lead_entity_type')?.updateValueAndValidity();

    this.leadDetailsForm.get('lead_business_started')?.clearValidators
    this.leadDetailsForm.get('lead_business_started')?.updateValueAndValidity();

    this.leadDetailsForm.get('lead_business_type')?.clearValidators
    this.leadDetailsForm.get('lead_business_type')?.updateValueAndValidity();

    this.officerInfoForm.get('officer_ssn')?.clearValidators();
    this.officerInfoForm.get('officer_ssn')?.updateValueAndValidity();

    this.officerInfoForm.get('officer_ownership')?.clearValidators();
    this.officerInfoForm.get('officer_ownership')?.updateValueAndValidity(); 

    this.officerInfoForm.get('officer_dob')?.clearValidators();
    this.officerInfoForm.get('officer_dob')?.updateValueAndValidity();
  }
}
}
