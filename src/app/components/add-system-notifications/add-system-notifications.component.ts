...
export class AddSystemNotificationsComponent implements OnInit {
	...
	async getTemplateDetails() {
		try {
			...
			this.commonService.showSpinner();
			const res$ = this.apiService.postReq(API_PATH.GET_SYSTEM_NOTIFICATION_TEMPLATE_DETAILS, data, 'system', 'notification-add');
			let response = await lastValueFrom(res$);
			if (response && response.data) {
				this.sendMailForm.patchValue({
	...
	async checkEmailSet() {
		try {
			...
			this.commonService.showSpinner();
			const res$ = this.apiService.postReq(API_PATH.EMAIL_EXIST_EMAIL_TEMPLATE, data, 'system', 'notification-add');
			let response = await lastValueFrom(res$);
			if (response && response.data) {
