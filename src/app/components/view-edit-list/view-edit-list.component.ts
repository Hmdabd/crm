...
import * as Model from './view-edit-list-model';

@Component({
    selector: 'app-view-edit-list',
    templateUrl: './view-edit-list.component.html',
    styleUrls: ['./view-edit-list.component.scss']
})
export class ViewEditListComponent implements OnInit {
    popUps: any = {
        actionsPopup: false,
 ...
 tds[index].style.setProperty('--custom',`${this.color}`);
    ...
    async changeStatus(status: any) {
        try {
            ...
            const res$ = this.apiService.postReq(API_PATH.DRIP_CAMPAIGN_UPDATE_STATUS, data, 'list', 'update');
    ...
    async getDripCampaignDetails(): Promise<void> {
        try {
            ...
            const res$ = this.apiService.getReq(API_PATH.VIEW_LISTING + url, 'list', 'view');
            let response = await lastValueFrom(res$);
            if (response && response.data && response.data.data) {
                this.dripCampaignDeleteStatus = response.data.data.isDeleted;
                this.campaignSteps = response.data.data.leadCompaignStep;
                this.dripCampaignName = response.data.data.name;
                this.selectList = response.data.data.select_list;
                ...
                for (let i = 0; i < this.campaignSteps.length; i++) {
                    var str1 = new String("Set property value(");
                    var str2 = this.campaignSteps[i].header_text;
                    var str3 = new String(")")
                    var str4 = str1.concat(str2.toString());
                    var str5 = str4.concat(str3.toString());
                    this.campaignSteps[i].header_text = str5
                }
                for (let i = 0; i < this.campaignSteps.length; i++) {
                    if (this.campaignSteps[i].header_text == 'Set property value(Lead Status)') {
                        this.setPropertyForm.patchValue({
                            leadStatus: this.campaignSteps[i].data.status_id
                        })
                        this.getLeadList();
                    }
                    else if (this.campaignSteps[i].header_text == (this.userDetails?.company_type == 'broker'? 'Set property value(Lead Source)':'Set property value(ISO)' )) {
                        this.setPropertyForm.patchValue({
                            leadSource: this.campaignSteps[i].data.status_id
                        })
                        this.getLeadList();
                    }
                    else if (this.campaignSteps[i].header_text == 'Set property value(Assigned To)') {
                        this.setPropertyForm.patchValue({
                            assignedTo: this.campaignSteps[i].data.status_id
                        })
                        this.getLeadList();
                    } else if (this.campaignSteps[i].header_text == 'Set property value(Created By)') {
                        this.setPropertyForm.patchValue({
                            ownerName: this.campaignSteps[i].data.status_name
                        })
                        this.getLeadList();
                    } else if (this.campaignSteps[i].header_text == 'Set property value(Company Name)') {
                        this.setPropertyForm.patchValue({
                            companyName: this.campaignSteps[i].data.status_name
                        })
                        this.getLeadList()
                    } else if (this.campaignSteps[i].header_text == 'Set property value(Lead No)') {
                        this.setPropertyForm.patchValue({
                            leadNo: this.campaignSteps[i].data.status_name
                        })
                        this.getLeadList()
                    } else if (this.campaignSteps[i].header_text == 'Set property value(Date)') {
                        this.setPropertyForm.patchValue({
                            date: this.campaignSteps[i].data.status_name
                        })
                        this.getLeadList()
                    } else if (this.campaignSteps[i].header_text == 'Set property value(Bussiness State)') {
                        this.setPropertyForm.patchValue({
                            country: this.campaignSteps[i].data.country_id,
                        })
                        if(this.setPropertyForm.value.country){
                               this.getStates(this.campaignSteps[i].data.country_id, this.campaignSteps[i].data.status_id)
                ...
                // this.leadList = response.data.data.leadList
    ...
    async getLeadList() {
        try {
            let data = {
                list_created_date: this.createdDate,
            ...
            if (this.setPropertyForm.value.leadStatus) {
                data.list_type = this.selectList,
                    data.list_created_date = this.createdDate,
    ...
    async onEndClick() {
        try {
            ...
            this.commonService.showSpinner();
            const res$ = this.apiService.postReq(API_PATH.UPDATE_LIST, data, 'list', 'add');
            let response = await lastValueFrom(res$);
            if (response) {
                this.commonService.showSuccess(response.message);
    ...
    removeStep(index: number, header: any) {
        try {
            if (header == 'Set property value(Lead Status)') {
                ...
                this.setPropertyForm.get('leadStatus')?.clearValidators()
                this.setPropertyForm.get('leadStatus')?.updateValueAndValidity();
            } else if (header == (this.userDetails?.company_type == 'broker'? 'Set property value(Lead Source)':'Set property value(ISO)' )) {
    ...
    async onSettingsSave() {
        try {
            ...
            this.commonService.showSpinner();
            const res$ = this.apiService.postReq(API_PATH.DRIP_CAMPAIGN_UPDATE, data, 'list', 'update');
            let response = await lastValueFrom(res$);
            if (response) {
                this.commonService.showSuccess(response.message);
    ...
    openModal(templateRef: TemplateRef<any>) {
        try {
            ...
            // 
            if(this.selectList == 'active'){
                this.CheckBoxCondition = true;
                }else{
                    this.CheckBoxCondition = false;
                }
                if(this.selectList == 'static'){
                    ...
                    if (tempLeadSource) {
                        data.status_id = this.setPropertyForm.value.leadSource,
                            ...
                            if(this.userDetails.company_type == 'broker'){
                                ...
                                data.header_text = `Set property value(ISO)`;
                            }
    ...
    editsetProperty(){ 
        ...
        else if(this.campaignSteps[0].header_text == (this.userDetails?.company_type == 'broker'? 'Set property value(Lead Source)':'Set property value(ISO)') ){
            this.setPropertyForm.patchValue({
