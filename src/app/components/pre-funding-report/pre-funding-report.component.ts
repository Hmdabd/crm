...
@Component({
    selector: 'app-pre-funding-report',
    templateUrl: './pre-funding-report.component.html',
    styleUrls: ['./pre-funding-report.component.scss', '../../styles/dashboard.scss']
})
export class PreFundingReportComponent implements OnInit {
    pageLimit: number = 10;
...
for (let index = 0; index < tds.length; index++) {
 tds[index].style.setProperty('--custom',`${this.color}`);
    ...
    async getFundingReports() {
        ...
        if (this.appliedFilter['Partner Status']?.value && this.appliedFilter['Partner Status'].value.name) {
            url = `${url}&agent_status=${this.appliedFilter['Partner Status']?.value.name}`;
        }
