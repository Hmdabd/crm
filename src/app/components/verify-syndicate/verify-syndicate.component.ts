...
@Component({
  selector: 'app-verify-syndicate',
  templateUrl: './verify-syndicate.component.html',
  styleUrls: ['./verify-syndicate.component.scss']
})
export class VerifySyndicateComponent implements OnInit {
  token: string = '';
  color: string = '#fa5440';
  exactColor: string = '#fa5440';
    ...
    async verifyToken() {
        try {
            const res$ = this.apiService.postReq(API_PATH.VERIFY_SYNDICATE, { token: this.token, color: this.exactColor }, '', '');
    ...
    saveUserDetails(response: any): void {
        ...
        this.router.navigate([`/syndicate/syndicate-update/${response.data.syndicate_id}`], { queryParams:{token: response.data.token }})
    }
    // loader color
    ngDoCheck(): void {
