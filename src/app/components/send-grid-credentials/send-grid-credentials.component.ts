...
export class SendGridCredentialsComponent implements OnInit {
    ...
    initAddHelloSignForm(): void {
        this.addHelloSignForm = this.fb.group({
            ...
            hello_sign_api_key: [''],
            hub_sign_api_key: [''],
            hello_sign_enviroment: [0],
            hello_sign_status: [false],
            hub_sign_status: [false]
    ...
    async addHelloSignSubmit(): Promise<void> {
        if(this.companyType == 'funded'){  
            this.addHelloSignForm.get('signtype')?.setValidators([Validators.required]);
            this.addHelloSignForm.get('signtype')?.updateValueAndValidity();
            this.addHelloSignForm.get('signtype')?.markAsTouched();
         if (this.addHelloSignForm.value.signtype == 'HelloSign') {
            this.addHelloSignForm.get('hub_sign_api_key')?.clearValidators();
            this.addHelloSignForm.get('hub_sign_api_key')?.updateValueAndValidity();
            this.addHelloSignForm.get('hub_sign_status')?.clearValidators()
            this.addHelloSignForm.get('hub_sign_status')?.updateValueAndValidity();
            this.addHelloSignForm.get('hello_sign_api_key')?.setValidators([Validators.required, Validators.pattern(Custom_Regex.spaces)]);
            this.addHelloSignForm.get('hello_sign_enviroment')?.setValidators([Validators.required]);
            this.addHelloSignForm.get('hello_sign_api_key')?.updateValueAndValidity();
            this.addHelloSignForm.get('hello_sign_api_key')?.markAsTouched();
            this.addHelloSignForm.get('hello_sign_enviroment')?.updateValueAndValidity();
            this.addHelloSignForm.get('hello_sign_enviroment')?.markAsTouched();
        } else {
            this.addHelloSignForm.get('hub_sign_api_key')?.setValidators([Validators.required, Validators.pattern(Custom_Regex.spaces)]);
            this.addHelloSignForm.get('hub_sign_api_key')?.updateValueAndValidity();
            this.addHelloSignForm.get('hub_sign_api_key')?.markAsTouched();
            this.addHelloSignForm.get('hello_sign_api_key')?.clearValidators();
            this.addHelloSignForm.get('hello_sign_api_key')?.updateValueAndValidity();
            this.addHelloSignForm.get('hello_sign_status')?.clearValidators()
            this.addHelloSignForm.get('hello_sign_status')?.updateValueAndValidity();
            this.addHelloSignForm.get('hello_sign_enviroment')?.clearValidators()
            this.addHelloSignForm.get('hello_sign_enviroment')?.updateValueAndValidity();
       
        }}else{
            this.addHelloSignForm.get('signtype')?.clearValidators();
            this.addHelloSignForm.get('signtype')?.updateValueAndValidity();
            this.addHelloSignForm.get('hub_sign_status')?.clearValidators()
            this.addHelloSignForm.get('hub_sign_status')?.updateValueAndValidity();
            this.addHelloSignForm.get('hub_sign_api_key')?.clearValidators();
        ...
        if (this.addHelloSignForm.valid) {
           ...
           if(this.companyType == 'funded'){  
                if (this.addHelloSignForm.value.signtype == 'HelloSign') {
                    data = {
                        service: 'helloSign',
                        hello_sign_api_key: this.addHelloSignForm.value.hello_sign_api_key,
                        hello_sign_enviroment: this.addHelloSignForm.value.hello_sign_enviroment,
                        hello_sign_status: this.addHelloSignForm.value.hello_sign_status,
                    }
                } else {
                    data = {
                        service: 'hubSign',
                        hub_sign_api_key: this.addHelloSignForm.value.hub_sign_api_key,
                        hub_sign_status: this.addHelloSignForm.value.hub_sign_status,
                    }
                } }else{
                    data = {
                        service: 'helloSign',
    ...
    patchHelloSignValues() {
        ...
        // })
        this.addHelloSignForm.patchValue({
            hello_sign_api_key: this.hellosignConfigurationsDetails?.helloSign.hello_sign_api_key,
            hello_sign_enviroment: this.hellosignConfigurationsDetails?.helloSign.hello_sign_enviroment,
            hello_sign_status: this.hellosignConfigurationsDetails?.helloSign.hello_sign_status,
            signtype:  this.hellosignConfigurationsDetails?.helloSign.hello_sign_status ? 'HelloSign' : this.hellosignConfigurationsDetails?.hubSign.hub_sign_status ? 'HubSign' : 'HelloSign',
            hub_sign_api_key: this.hellosignConfigurationsDetails?.hubSign.hub_sign_api_key,
            hub_sign_status: this.hellosignConfigurationsDetails?.hubSign.hub_sign_status,
        });
    }
    ...
    patchEmailConfigValues() {
        this.addUserForm.patchValue({
            ...
            type: this.emailConfigurationsDetails?.SendGrid.sendgrid_status ? 'SendGrid' : this.emailConfigurationsDetails?.smtp.smtp_status ? 'SMTP' : 'SendGrid',
    ...
    changeHelloSignStatus() {
        if (this.addHelloSignForm.value.hello_sign_status == true) {
            Swal.fire({
                title: 'Are you sure want to enabled hello sign status and disabled hub sign status',
                icon: 'warning',
                showCancelButton: true,
                confirmButtonText: 'OK',
                confirmButtonColor: this.color,
    ...
    changeHubSignStatus() {
        if (this.addHelloSignForm.value.hub_sign_status == true) {
            Swal.fire({
                title: 'Are you sure want to enabled hub sign status and disabled hello sign status',
                icon: 'warning',
                showCancelButton: true,
                confirmButtonText: 'OK',
                confirmButtonColor: this.color,
