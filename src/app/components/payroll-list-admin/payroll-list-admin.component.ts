...
@Component({
  selector: 'app-payroll-list-admin',
  templateUrl: './payroll-list-admin.component.html',
  styleUrls: [ '../../styles/dashboard.scss',
  '../../styles/predictive-search.scss','../payroll-list/payroll-list.component.scss','./payroll-list-admin.component.scss']
})
export class PayrollListAdminComponent implements OnInit {
  pageLimit: number = 10;
...
async getUsersList(): Promise<any> {
    try {
        ...
        // }
        const res$ = this.apiService.getReq(
            API_PATH.GET_PAYROLL_LIST_ADMIN + url,
            'payroll',
            'list'
        );
        let response = await lastValueFrom(res$);
        ...
        if (response && response.data && response.data.data) {
            ...
            console.log(this.usersList,'Updates');
...
opencompanyPaidpopup(id: any, status: any, statusType: any) {
    Swal.fire({
        ...
        if (result.value) {
            this.companyPaidStatus(id, status);
...
async companyPaidStatus(id: any, status: any): Promise<void> {
    if (status == 'paid') {
        status = 'unpaid'
    } else if (status == 'unpaid') {
        status = 'paid'
    }
    let url = `?id=${id}&paid_status=${status}`;
    try {
        this.commonService.showSpinner();
        const res$ = this.apiService.getReq(API_PATH.PAYROLL_ADMIN_COMPANY_STATUS + url, 'lead', 'pre-funding');
        let response = await lastValueFrom(res$);
        if (response && response.data) {
            this.commonService.showSuccess(response.message);
            this.getUsersList();
...
getamount(data: any, upfront_commission: any, placeholder: any) {
    let value = 0;
    if (placeholder.agent_type == 'Assigned ISO') {
...
async getLeadsDataForExport(url: string) {
    try {
        ...
        this.commonService.showSpinner();
        const res$ = this.apiService.postReq(API_PATH.PAYROLL_EXPORT_ADMIN, { export_payroll: this.payrollIdArray }, 'payroll', 'export');
...
getPercentage(data:any){
    ...
    return '(' + ((final_total * 100)/totalEarning).toFixed(2) + ')' + '%'
    } else {
        return ''
    }
...
async getuserListing() {
    try {
        let url = `?page_limit=2000&page=${this.userListingPage}&status=Active`;
        this.commonService.showSpinner();
