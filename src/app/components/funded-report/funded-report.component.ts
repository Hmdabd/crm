...
@Component({
    selector: 'app-funded-report',
    templateUrl: './funded-report.component.html',
    styleUrls: ['./funded-report.component.scss', '../../styles/dashboard.scss']
})
export class FundedReportComponent implements OnInit {
    pageLimit: number = 10;
...
for (let index = 0; index < tds.length; index++) {
 tds[index].style.setProperty('--custom',`${this.color}`);
    ...
    async getFundingReports() {
        ...
        if (this.appliedFilter['Percentage']?.value && this.appliedFilter['Percentage'].value.name) {
            url = `${url}&percentage=${this.appliedFilter['Percentage'].value.name}`;
          }
        try {
            this.commonService.showSpinner();
            const res$ = this.apiService.getReq(API_PATH.FUNDED_REPORT + url, 'funded', 'report');
            let response = await lastValueFrom(res$);
            if (response && response.status_code == "200") {
                if (response.data && response.data.data) {
                    this.fundedReportList = response.data.data;
                    this.totalRecords = response.data.total_records;
                } else {
                    this.fundedReportList = [];
                }
            }
            this.commonService.hideSpinner();
        } catch (error: any) {
    ...
    async exportReportasExcel() {
        try {
            ...
            if (res.length) {
                this.excelService.exportAsExcelFile(res, 'Funded_Report');
            } else {
    ...
    async getReportDataForExport() {
        try {
            ...
            this.commonService.showSpinner();
            const res$ = this.apiService.getReq(API_PATH.FUNDED_REPORT + url, 'funding', 'report');
    ...
    sortBy(col:string){
        if(!this.fundedReportList.length){
           return;
        }
        ...
        getlength(FundDeatil:any){
           return Object.keys(FundDeatil).length;
        }
}
