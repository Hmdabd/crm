...
@Component({
  selector: 'app-verify-lead-source',
  templateUrl: './verify-lead-source.component.html',
  styleUrls: ['./verify-lead-source.component.scss']
})
export class VerifyLeadSourceComponent implements OnInit {
  token: string = '';
  color: string = '#fa5440';
  exactColor: string = '#fa5440';
    ...
    async verifyToken() {
        try {
            const res$ = this.apiService.postReq(API_PATH.VERIFY_LEAD_SOURCE, { token: this.token, color: this.exactColor }, '', '');
    ...
    saveUserDetails(response: any): void {
        ...
        this.router.navigate([`/leadsource/lead-source-update/${response.data.source_id}`], { queryParams:{token: response.data.token }})
    }
    // loader color
    ngDoCheck(): void {
