...
@Component({
  selector: 'app-verify-lender-offer',
  templateUrl: './verify-lender-offer.component.html',
  styleUrls: ['./verify-lender-offer.component.scss']
})
export class VerifyLenderOfferComponent implements OnInit {
  ...
  exactColor: string = '#fa5440';
    ...
    async verifyToken() {
        try {
            const res$ = this.apiService.postReq(API_PATH.VERIFY_LENDER_OFFER, { token: this.token, color: this.exactColor }, '', '');
    ...
    saveUserDetails(response: any): void {
        ...
        this.router.navigate([`/leadsource/lender-offer-update/${response.data.lender_offer_id}`], { queryParams:{token: response.data.token }})
    }
    // loader color
    ngDoCheck(): void {
