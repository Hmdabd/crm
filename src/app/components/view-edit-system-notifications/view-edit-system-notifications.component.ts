...
export class ViewEditSystemNotificationsComponent implements OnInit {
    ...
    closeAllModels() {
        ...
        if (this.sendMailForm.value.template_type === 'predefined') {
            ...
            this.sendMailForm.get('from_option')?.clearValidators()
            this.sendMailForm.get('from_option')?.updateValueAndValidity();
            this.sendMailForm.get('to_option')?.clearValidators()
            this.sendMailForm.get('to_option')?.updateValueAndValidity();
        } else {
            ...
            this.sendMailForm.get('to_option_scratch')?.clearValidators()
            this.sendMailForm.get('to_option_scratch')?.updateValueAndValidity();
            this.sendMailForm.get('from_option_scratch')?.clearValidators()
            this.sendMailForm.get('from_option_scratch')?.updateValueAndValidity();
    ...
    ontemplateSave() {
        try {
            ...
            if (this.sendMailForm.value.template_type === 'predefined') {
                ...
                this.sendMailForm.get('cc_template')?.updateValueAndValidity();
                this.sendMailForm.get('from_option')?.setValidators([Validators.required])
                this.sendMailForm.get('from_option')?.updateValueAndValidity();
                this.sendMailForm.get('to_option')?.setValidators([Validators.required])
                this.sendMailForm.get('to_option')?.updateValueAndValidity();
                ...
                this.sendMailForm.get('cc')?.updateValueAndValidity();
                this.sendMailForm.get('from_option_scratch')?.setValidators([Validators.required])
                this.sendMailForm.get('from_option_scratch')?.updateValueAndValidity();
                this.sendMailForm.get('to_option_scratch')?.setValidators([Validators.required])
                this.sendMailForm.get('to_option_scratch')?.updateValueAndValidity();
            }
    ...
    getemailPopup(i: any) {
        ...
        if (this.campaignSteps[i].data.template_type == "from_scratch") {
            ...
            // })
            this.emailtemplateCampaign(i);
        }

    }
    async emailtemplateCampaign(i: any){
            try {
                let data = {
                    template_id: this.campaignSteps[i].data.templateId,
                }
                this.commonService.showSpinner();
                const res$ = this.apiService.postReq(API_PATH.GET_SYSTEM_NOTIFICATION_TEMPLATE_DETAILS, data, 'system', 'notification-update');
                let response = await lastValueFrom(res$);
                if (response && response.data) {
                    // this.sendMailForm.patchValue({
                    //     from_option: response.data.from,
                    //     to_option: response.data.to
                    // })
                    this.sendMailForm.patchValue({
                        from_option: response.data.from,
                        cc_template: this.campaignSteps[i].data.cc_template,
    ...
    async getTemplateDetails() {
        try {
            let data = {
                template_id: this.sendMailForm.value.template,
            }
            this.commonService.showSpinner();
            const res$ = this.apiService.postReq(API_PATH.GET_SYSTEM_NOTIFICATION_TEMPLATE_DETAILS, data, 'system', 'notification-update');
            let response = await lastValueFrom(res$);
    ...
    async checkEmailSet() {
        try {
            let data = {
                from_option: this.sendMailForm.value.from_option_scratch,
            }
            this.commonService.showSpinner();
            const res$ = this.apiService.postReq(API_PATH.EMAIL_EXIST_EMAIL_TEMPLATE, data, 'system', 'notification-update');
            let response = await lastValueFrom(res$);
            if (response && response.data) {
