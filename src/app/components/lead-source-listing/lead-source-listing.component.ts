...
@Component({
    selector: 'app-lead-source-listing',
    templateUrl: './lead-source-listing.component.html',
    styleUrls: ['./lead-source-listing.component.scss', '../../styles/dashboard.scss', '../../styles/predictive-search.scss']
})
export class LeadSourceListingComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.canAddLeadSource = this.authService.hasPermission('ISO-create');
        this.canViewLeadList = this.authService.hasPermission('ISO-list');
        this.canUpdateLeadSource = this.authService.hasPermission('ISO-update');
        this.canDeleteLeadSource = this.authService.hasPermission('ISO-delete');
        this.canViewLeadSource = this.authService.hasPermission('ISO-view');
        this.canUpdateStatus = this.authService.hasPermission('ISO-update-status');
//         [ 'name' => 'ISO-list'],
...
//                     [ 'name' => 'ISO-update-status'],
  ...
  */
    ...
    async submitemailTemplate(): Promise<void> {
        ...
        if (this.emailTemplateId != '' && this.dripCampaignTriggerForm.valid) {
            ...
            // this.router.navigate([`/${this.userBaseRoute}/add-drip-campaign`], { queryParams: { name: this.dripCampaignTriggerForm.value.name, id: this.emailTemplateId } });
        }
  ...
  */
    ...
    async deleteLeadSource(user: any) {
        ...
        try {
            let url = `?id=${user.id}&name=${user.name}&type=${'ISO'}`;
            this.commonService.showSpinner();
            const res$ = this.apiService.getReq(API_PATH.DELETE_LEAD_SOURCE + url,  'ISO', 'delete');
            let response = await lastValueFrom(res$);
    ...
    */
    async getLeadSourceList(): Promise<any> {
        try {
            let url = `?sort_by=${this.search.sortby}&dir=${this.search.order}&page_limit=${this.usersListLimit}&page=${this.userListPage}&type=${'ISO'}`;
            // if (this.searchKeyword) {
            ...
            this.commonService.showSpinner();
            const res$ = this.apiService.getReq(API_PATH.LEAD_SOURCE_LIST + url,  'ISO', 'list');
            let response = await lastValueFrom(res$);
    ...
    openTriggerModal(templateRef: TemplateRef<any>) {
        try {
            ...
            this.dripCampaignTriggerForm.patchValue({
                name: this.leadSouceForm.value.name,
            })
    ...
    async updateStatus(status: string, user: any, input: any, i: number) {
        try {
            const res$ = this.apiService.postReq(API_PATH.UPDATE_STATUS_LEAD_SOURCE, { name: user.name, id: user.id, status: status, type:'ISO' }, 'ISO', 'update-status');
    ...
    editLeadModal(templateRef: TemplateRef<any>, user: any) {
        // this.closeTriggerModal();
        // this.emailTemplateId = id
        this.patchValue(user.name, user.id)
        try {
    ...
    async  checkEmailTemplateExists(model: any , actionType: any, isoId: any){
        this.commonService.showSpinner();
        try {
            const res$ = this.apiService.postReq(API_PATH.LEAD_EMAIL_TEMPLATE_EXIST, {action_type: actionType }, 'lead', 'view');
            let response = await lastValueFrom(res$);
            if (response && response.status_code == "200") {
                // this.commonService.showSuccess(response.message);
               if(actionType == 'ISO' && model == 'create'){
                    this.router.navigate([`/${this.userBaseRoute}/create-lead-source`]);
                }  else if(actionType == 'ISO' && model == 'edit'){
                    this.router.navigate([`/${this.userBaseRoute}/edit-lead-source/` + isoId], { queryParams: { mode: 'edit'}})
                }  
