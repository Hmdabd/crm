...
@Component({
  selector: 'app-verify-participants',
  templateUrl: './verify-participants.component.html',
  styleUrls: ['./verify-participants.component.scss']
})
export class VerifyParticipantsComponent implements OnInit {
  token: string = '';
  color: string = '#fa5440';
  exactColor: string = '#fa5440';
    ...
    async verifyToken() {
        try {
            const res$ = this.apiService.postReq(API_PATH.VERIFY_PARTICIPANT_TOKEN, { token: this.token, color: this.exactColor }, '', '');
    ...
    saveUserDetails(response: any): void {
        ...
        this.router.navigate([`/syndicate/participant-update/${response.data.syndicate_id}`], { queryParams:{token: response.data.token,lead_id: response.data.lead_id}})
    }
    // loader color
    ngDoCheck(): void {
