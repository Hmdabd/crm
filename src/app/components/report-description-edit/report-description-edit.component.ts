...
export class ReportDescriptionEditComponent implements OnInit {
    ...
    initForm() {
        ...
        for (let i = 0; i < this.reportDescriptionArray.length; i++) {
            this.form.addControl(`${this.reportDescriptionArray[i].name}`, new FormControl(`${this.reportDescriptionArray[i].value}`, [Validators.required, Validators.pattern(Custom_Regex.spaces),Validators.pattern(Custom_Regex.address), Validators.pattern(Custom_Regex.address2), Validators.maxLength(500),
