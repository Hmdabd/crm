...
@Component({
    ...
    styleUrls: ['./drip-campaign.component.scss', '../../styles/dashboard.scss', '../../styles/predictive-search.scss']
})
export class DripCampaignComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.canViewDripCampaign = this.authService.hasPermission('campaign-view');
        this.canEditDripCampaign = this.authService.hasPermission('campaign-update');
        this.canDeleteDripCampaign = this.authService.hasPermission('campaign-delete');
  ...
  */
    ...
    async emailTemplateSubmit(): Promise<void> {
        ...
        if (this.dripCampaignForm.valid) {
            this.closeModal();
            this.router.navigate([`/${this.userBaseRoute}/drip-campaign-list/add-drip-campaign`], { queryParams: { name: this.dripCampaignForm.value.name } });
        }
        // else {
    ...
    */
    async getUsersList(): Promise<any> {
        try {
            ...
            const res$ = this.apiService.getReq(API_PATH.DRIP_CAMPAIGN_LIST + url, 'campaign', 'list');
            let response = await lastValueFrom(res$);
            if (response && response.data) {
                if (response.data.data) {
                    this.hasMoreUsers = response.data.hasMorePages;
                    this.totalUsersCount = response.data.total;
                    this.dripCampaignList = response.data.data;
                    this.dripCampaignList.forEach(object => { object.toggle = false })
                    for (let i = 0; i < this.dripCampaignList.length; i++) {
                        if (this.dripCampaignList[i].status == 'Active') {
                            this.dripCampaignList[i].toggle = true;
                        } else {
                            this.dripCampaignList[i].toggle = false;
                        }
                    }
                } else {
  ...
  */
    ...
    async deleteDripcampaign(user: any) {
        try {
            ...
            if (response && response.status_code == "200") {
                ...
                let obj = this.dripCampaignList.filter(e => e.id == user.id);
    ...
    async updateStatus(status: string, user: any, input: any, i: number) {
        try {
            const res$ = this.apiService.postReq(API_PATH.DRIP_CAMPAIGN_UPDATE_STATUS, { name: user.name, id: user.id, status: status }, 'campaign', 'update');
