...
export class AddSyndicateComponent implements OnInit {
        ...
        if (this.addUserForm.valid) {
            ...
            try {
                ...
                let data = {
                    ...
                    syndicate_id: this.previewData?.syndicate_id

                }
                // if(this.previewData.syndicate_id){

                // }
                const res$ = this.apiService.postReq(API_PATH.CREATE_LEAD_SOURCE, data, 'syndicate', 'add');
                let response = await lastValueFrom(res$);
                if (response) {
                    this.commonService.showSuccess(response.message);
    ...
    */
        ...
        try {
            ...
            if (this.formFileArray.length) {
                ...
                if (this.formFileArray.valid) {
                    ...
                    formData.append('syndicate_id', this.previewData?.syndicate_id);
                    this.uploading = true;
                    this.ngxLoader.startLoader('loader1');

                    const res$ = this.apiService.postReq(API_PATH.SYNDICATE_DOCUMENT_UPLOAD, formData, 'syndicate', 'add');
                    const response = await lastValueFrom(res$);
                    if (response && response.status_code == "200") {
                        this.commonService.showSuccess(response.message);
                        this.uploading = false;
    ...
    async getLeadSourceDocument() {
        try {
            this.commonService.showSpinner();
            let url = `?records_per_page=${'1000'}&page=${'1'}&syndicate_id=${this.previewData?.syndicate_id}`;
            // let reqData: any = {
            //     page: 1,
            ...
            const res$ = this.apiService.getReq(API_PATH.SYNDICATE_UPLOADED_DOCUMENTS_LIST + url, 'syndicate', 'add');
            let response = await lastValueFrom(res$);
            if (response && response.status_code == "200") {
    ...
    async documentsDelete(id: any): Promise<void> {
        ...
        try {
            this.commonService.showSpinner();
            const res$ = this.apiService.postReq(API_PATH.SYNDICATE_DOCUMENT_DELETE, reqData, 'syndicate', 'add');
            let response = await lastValueFrom(res$);
            if (response && response.data) {
        ...
        if (this.addUserForm.valid) {
            ...
            try {
                ...
                if (response) {
                    this.commonService.showSuccess(response.message);
                    this.previewData = response.data;
                    this.syndicateID = this.previewData?.syndicate_id
                    this.tabActive = {
                        id: 'OfficerInfo',
                        number: 2
                    }
    ...
    async onFinalStepSubmit() {
        ...
        if (this.filesForm.valid) {
            try {
                ...
                const res$ = this.apiService.postReq(API_PATH.SYNDICATE_EMAIL, reqData, 'syndicate', 'add');
                let response = await lastValueFrom(res$);
                if (response && response.status_code == "200") {
                    this.commonService.showSuccess(response.message);
                    this.router.navigate([`/${this.userBaseRoute}/syndicates`]);
