...
getColorOnUpdate() {
   this.colorSubs = this.authService.getColor().subscribe((u) => {
    ...
    initForm() {
        for (let i = 0; i < this.permissions.length; i++) {
            this.form.addControl(`${this.permissions[i].name}`, new FormControl(`${this.permissions[i].description}`, [Validators.required, Validators.pattern(Custom_Regex.spaces), Validators.maxLength(500),
