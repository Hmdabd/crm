...
export class SyndicatesListComponent implements OnInit {
  ...
  */
    ...
    onDateSelection(date: NgbDate) {
      ...
      /**
    * @description get syndicate list eg syndicate list if adminisrator is logged in shown individual.
    */
    ...
    async  checkEmailTemplateExists(model: any , actionType: any, syndicateId: any){
        this.commonService.showSpinner();
        try {
            ...
            if (response && response.status_code == "200") {
                // this.commonService.showSuccess(response.message);
               if(actionType == 'Syndicate' && model == 'create'){
                    this.router.navigate([`/${this.userBaseRoute}/syndicates/add`]);
                }  else if(actionType == 'Syndicate' && model == 'edit'){
                    this.router.navigate([`/${this.userBaseRoute}/syndicates/` + syndicateId], { queryParams: { mode: 'edit'}})
                }    else if(actionType == 'Syndicate' && model == 'view'){
                    this.router.navigate([`/${this.userBaseRoute}/syndicates/` + syndicateId], { queryParams: { mode: 'view'}})
                }      
